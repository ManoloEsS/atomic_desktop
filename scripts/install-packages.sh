...
provision_vendor_repo() {
  ...
  check_key_fingerprint "$key_tmp" "$expected_fpr" "Vendor $package"
  rm -f -- "$key_tmp"
  trap - RETURN
  ...
  temporary=$(mktemp)
  trap 'rm -f -- "${temporary:-}"' RETURN
  curl --fail --silent --show-error --location "$repo_url" --output "$temporary"
  grep -q 'gpgkey' "$temporary" || die "downloaded repository for $package lacks gpgkey; refusing to use it"

  if run_root test -r "$destination" && run_root cmp --silent "$temporary" "$destination"; then
    info "Vendor repository already configured: $package"
  elif run_root test -e "$destination"; then
    die "$destination differs from the expected vendor repository for $package; inspect it before continuing"
  else
    run_root install -o root -g root -m 0644 "$temporary" "$destination"
    info "Configured vendor repository: $package"
  fi
  rm -f -- "$temporary"
  trap - RETURN
}
...
done < "$VENDOR_REPOSITORY_MANIFEST"
